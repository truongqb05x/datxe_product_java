...
<body>
    ...
    <main class="main-content">
        ...
        <div class="kpi-cards">
            ...
            <div class="kpi-card kpi-vehicles">
                ...
                <div class="kpi-info">
                    ...
                    <div class="kpi-value" id="kpiUtilization"><fmt:formatNumber value="${vehicleUtilization}" pattern="0.0"/>%</div>
        ...
        <section class="report-filters">
            ...
            <form action="${pageContext.request.contextPath}/admin/reports" method="GET" id="reportFilterForm">
                <div class="filter-grid">
                    <div class="filter-group">
                        <label for="reportPeriod">Kỳ báo cáo</label>
                        <select id="reportPeriod" name="period">
                            <option value="monthly" ${param.period eq 'monthly' ? 'selected' : ''}>Theo tháng</option>
                            <option value="quarterly" ${param.period eq 'quarterly' ? 'selected' : ''}>Theo quý</option>
                            <option value="yearly" ${param.period eq 'yearly' ? 'selected' : ''}>Theo năm</option>
                            <option value="custom" ${param.period eq 'custom' ? 'selected' : ''}>Tùy chỉnh</option>
                        </select>
                    </div>
                    ...
                    <div class="filter-group">
                        ...
                        <select id="reportCategory" name="category">
                            <option value="all" ${empty param.category or param.category eq 'all' ? 'selected' : ''}>Tất cả loại xe</option>
                            <option value="motorcycle" ${param.category eq 'motorcycle' ? 'selected' : ''}>Xe máy</option>
                            <option value="electric" ${param.category eq 'electric' ? 'selected' : ''}>Xe điện</option>
                            <option value="car" ${param.category eq 'car' ? 'selected' : ''}>Ô tô</option>
                        </select>
                    </div>
                    ...
                    <div class="filter-group">
                        ...
                        <select id="reportMetrics" name="metrics">
                            <option value="revenue" ${param.metrics eq 'revenue' ? 'selected' : ''}>Doanh thu</option>
                            <option value="bookings" ${param.metrics eq 'bookings' ? 'selected' : ''}>Số đơn đặt</option>
                            <option value="utilization" ${param.metrics eq 'utilization' ? 'selected' : ''}>Tỷ lệ sử dụng</option>
                            <option value="customers" ${param.metrics eq 'customers' ? 'selected' : ''}>Khách hàng</option>
                        </select>
                    </div>
                </div>
                        ...
                        <div class="comparison-item">
                            <div class="item-info">
                                ...
                                <div class="item-details">
                                    <h4>${vehicle.name}</h4>
                                    <p>${vehicle.bookings} đơn • <fmt:formatNumber value="${vehicle.utilization}" pattern="0.0"/>% sử dụng</p>
                                </div>
                            </div>
                            ...
                            <div class="item-info">
                                ...
                                <div class="item-details">
                                    ...
                                    <p>${customer.bookings} đơn • 
                                       <fmt:formatNumber value="${customer.avgSpent}" type="currency" currencySymbol="₫" maxFractionDigits="0"/>/đơn</p>
                                </div>
                            </div>
                            <div class="item-stats">
                                <div class="item-value">
                                ...
                                <div class="progress-bar" style="margin-top: 0.3rem;">
                                    ...
                                    <div class="progress-fill ${loyaltyClass}" style="width: ${customer.loyalty}%"></div>
                                </div>
                        ...
                        <tbody id="revenueTableBody">
                            <c:forEach var="item" items="${monthlyRevenueData}">
                                <tr>
                                    ...
                                    <td><fmt:formatNumber value="${item.revenue}" type="currency" currencySymbol="₫" maxFractionDigits="0"/></td>
                                    <td>
                                        <span class="${item.growth >= 0 ? 'positive' : 'negative'}">
                                            <i class="fas fa-${item.growth >= 0 ? 'arrow-up' : 'arrow-down'}"></i>
                                            ${Math.abs(item.growth)}%
                                        </span>
                                    </td>
                                    <td>${item.bookings}</td>
                                    <td><fmt:formatNumber value="${item.avgBooking}" type="currency" currencySymbol="₫" maxFractionDigits="0"/></td>
                                    <td>
                                        <div class="progress-bar">
                                            <div class="progress-fill progress-primary" style="width: ${Math.min(100, item.growth + 20)}%"></div>
                                        </div>
                                    </td>
                                </tr>
                            </c:forEach>
                        ...
                        <tbody id="categoryTableBody">
                            <c:forEach var="category" items="${categoryData}">
                                <tr>
                                    ...
                                    <td><fmt:formatNumber value="${category.revenue}" type="currency" currencySymbol="₫" maxFractionDigits="0"/></td>
                                    <td><fmt:formatNumber value="${category.percentage}" pattern="0.0"/>%</td>
                                    <td>${category.bookings}</td>
                                    <td><fmt:formatNumber value="${category.avgDaily}" type="currency" currencySymbol="₫" maxFractionDigits="0"/></td>
                                    <td>
                                        <div class="progress-bar">
            ...
            <div class="insights-grid">
                <div class="insight-card success">
                    ...
                    <div class="insight-content">
                        <p>Doanh thu tháng ${currentMonth} tăng <strong>${revenueChange}%</strong> so với tháng trước. 
        ...
        <div class="modal" id="customReportModal">
            <div class="modal-content" style="max-width: 700px;">
                ...
                <div class="modal-body">
                    <form id="customReportForm" action="${pageContext.request.contextPath}/admin/reports/export" method="POST">
                        <div style="display: grid; gap: 1.5rem;">
                            ...
                            <div class="filter-group">
                                ...
                                <div style="display: grid; gap: 0.5rem;">
                                    <label style="display: flex; align-items: center; gap: 0.5rem;">
                                        <input type="checkbox" name="includeCharts" id="includeCharts"> Bao gồm biểu đồ
                                    </label>
                                    <label style="display: flex; align-items: center; gap: 0.5rem;">
                                        <input type="checkbox" name="includeInsights" id="includeInsights"> Bao gồm insights
                                    </label>
                                    <label style="display: flex; align-items: center; gap: 0.5rem;">
                                        <input type="checkbox" name="scheduleReport" id="scheduleReport"> Lên lịch gửi hàng tuần
                                    </label>
                                </div>
                            </div>
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" id="generateCustomReportBtn" type="submit" form="customReportForm">Tạo báo cáo</button>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        function initRevenueChart(chartType = 'revenue-monthly') {
            ...
            if (chartType === 'revenue-monthly') {
                ...
                data = [32.5, 35.2, 38.7, 40.1, 39.8, 42.5, 
                       45.2, 44.8, 47.3, 46.5, 48.2, 50.5];
